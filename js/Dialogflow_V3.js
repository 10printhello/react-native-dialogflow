...
export class Dialogflow_V3 {
    ...
    setParameters(parameters) {
        ...
        Object.entries(parameters).forEach(([key, v]) => {
            const _key = key
            const _new = { _key : value.encode(parameters[key])}
    ...
    normalizeContext(context) {
        ...
        if (!context.name.startsWith("projects/")) {
            context.name = "projects/" + this.projectId + "/locations/" + this.locationId + "/agents/" + this.agentId + "/sessions/" + this.sessionId + "/contexts/" + context.name;
        }
    ...
    requestEvent = async (eventName, eventParameters, onResult, onError) => {
        ...
        fetch(DEFAULT_BASE_URL + this.projectId + "/locations/" + this.locationId + "/agents/" + this.agentId + "/sessions/" + this.sessionId + ":detectIntent", {
            method: "POST",
            headers: {
